...
def postprocess(logits, pred_boxes, target_sizes=None, threshold=0.3, topk=100):
    """
    Args:
        logits:
        pred_boxes:
    ...
    k_value = min(topk, prob.shape[1])
    topk_v, topk_idx = paddle.topk(prob, k_value, axis=1)
    scores = topk_v
    topk_boxes = paddle.divide(topk_idx, paddle.to_tensor(logits.shape[2]))
    labels = topk_idx % logits.shape[2]
    ...
    boxes = paddle.take_along_axis(boxes, topk_boxes.unsqueeze(-1).expand([1, -1, 4]), 1)
...
def main():
    ...
    # create model
    deformable_detr_model = DeformableDetr(embed_dim=256,
                                           ffn_dim=1024,
                                           num_heads=8,
                                           num_points=4,
                                           ...
                                           num_queries=300,
                                           num_classes=91)
    deformable_detr_model.eval()
    # load weights
    weight_path = './deformable_detr_r50.pdparams'
    state_dict = paddle.load(weight_path)
    deformable_detr_model.set_state_dict(state_dict)
    # inference
    outputs = deformable_detr_model(input_pixels, input_mask)
    # postprocess
