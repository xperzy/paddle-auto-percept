...
Support caffe and torch style for residual blocks
Mostly refered:
...
class BottleneckBlock(nn.Layer):
    ...
    def __init__(self,
        ...
        # style: pytorch Conv1(kernel=1, stride=1, padding=0); Conv2(kernel=3, stride=2, padding=1)
        # style: caffe   Conv1(kernel=1, stride=2, padding=0); Conv2(kernel=3, stride=1, padding=1)
        conv1_stride = 1 if style=='pytorch' else stride
        conv2_stride = stride if style=='pytorch' else 1

        self.conv1 = nn.Conv2D(inplanes, width, 1, stride=conv1_stride, weight_attr=w_attr_2, bias_attr=False)
...
class ResNet(nn.Layer):
    def __init__(self,
        ...
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], backbone_lr=backbone_lr, style=style)
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], backbone_lr=backbone_lr, style=style)
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], backbone_lr=backbone_lr, style=style)
        if with_pool:
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, backbone_lr=1.0, style='pytorch'):
        ...
        layers.append(
            block(self.inplanes, planes, stride, downsample, 1, 64,
                  previous_dilation, norm_layer, backbone_lr=backbone_lr, style=style))
