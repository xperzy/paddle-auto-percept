...
class PositionEmbeddingLearned(nn.Layer):
    def __init__(self, num_position_features=256, row_embed_dim=50, col_embed_dim=50):
        super().__init__()
        w_attr_1 = self.init_weights()
        self.row_embed = nn.Embedding(row_embed_dim, num_position_features, weight_attr=w_attr_1)
        w_attr_2 = self.init_weights()
        self.col_embed = nn.Embedding(col_embed_dim, num_position_features, weight_attr=w_attr_2)
    ...
    def forward(self, tensor_list):
        ...
        pos = pos.expand([x.shape[0]] + pos.shape[1::]) #[batch, dim, h, w]
        return pos


def build_position_encoding(embed_dim=256, mode='sine', row_embed_dim=50, col_embed_dim=50):
    ...
    if mode == 'sine':
        ...
        position_embedding = PositionEmbeddingLearned(N_steps, row_embed_dim, col_embed_dim)
